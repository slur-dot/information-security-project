...
function Register() {
	...
	async function onSubmit(e) {
		...
		setBusy(true);
		try {
			// Generate identity signing keypair before registration; upload public JWK
			const { publicSigningKeyJwk } = await ensureIdentityKeys(username);
			await api.post('/auth/register', { username, password, publicSigningKeyJwk });
			// Auto-login
			...
			<div className="card animate-scale-in" style={{ maxWidth: '420px', width: '100%' }}>
				...
				<div style={{
					...
					<p style={{ color: 'var(--color-text-muted)', fontSize: '0.875rem' }}>
						...
						<Link to="/login" style={{ color: 'var(--color-text-accent)', textDecoration: 'none', fontWeight: 500 }}>
							Sign in
						</Link>
					</p>
				</div>
